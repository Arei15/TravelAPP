...
export default function PlaceCardViewModal() {
  ...
  const placeImages = [profileImage, placeImage, profileImage, placeImage];
  const [placeImageView, setPlaceImageView] = useState(placeImages[0]);

  const subtitle =
    "The Berlin Dom, or Berlin Cathedral, is one of the most iconic landmarks in the heart of Germany's capital city. This grandiose building.";
  const subtitleWordsLength = subtitle.split(" ");
  const rateReview = 4.3;
      ...
      >
        <div className="w-full overflow-y-auto h-screen">
          <div className="w-full flex justify-center relative h-max">
            ...
            <button className="absolute py-2.5 px-3 bg-[#323131]/40 rounded-[9px] top-16 right-3">
              <BookmarkSvg className="w-[22px] h-6 [&_path]:stroke-white" />
            </button>
            <span className="bottom-20 bg-white/60 rounded-2xl absolute p-3 flex gap-5">
              {placeImages.map((image, idx) => (
                <span
                  onClick={() => setPlaceImageView(image)}
                  key={idx}
                  className="border-4 border-white w-[54px] h-[54px] rounded-xl"
                >
                  <Image
                    src={image}
                    alt="place-name"
            ...
            <div className="w-full absolute top-[550px] rounded-t-[20px] bg-white flex flex-col pt-5 pb-28 gap-2.5">
              <span className="w-full flex items-center justify-between px-4">
                <h3 className="text-xl font-semibold">Berlin Dom</h3>
              ...
              <span className="flex px-4">
                ...
                <p className="text-[#848282]">
                  Am Lustgarten, 10178 Berlin, Германия
                </p>
              </span>

              <div className="pt-4 pb-1 gap-5 flex items-center px-4">
              ...
              {placeInfoMenu === "Overview" ? (
                <>
                  <div className="px-4">
                    <p className="text-sm">
                      {subtitleWordsLength.length > 20 && !isSeeMoreOpen
                        ? subtitleWordsLength.slice(0, 20).join(" ")
                        : subtitle}
                      <span
                        onClick={() => setIsSeeMoreOpen(!isSeeMoreOpen)}
                        className={clsx(
                          "text-sm text-main-blue font-medium cursor-pointer",
                          subtitleWordsLength <= 20 ? "hidden" : "block"
                        )}
                      >
                  ...
                  <span className="w-full flex justify-between border-b px-4 border-b-[#D9D9D9]/50 py-2.5">
                    <h5 className="text-sm font-semibold">Business Hours</h5>
                  ...
                  <span className="w-full flex border-b border-b-[#D9D9D9]/50 pb-2.5 items-center gap-1.5 px-4">
                    ...
                    <p className="text-[#848282]">+1 (555) 555-5555</p>
                  </span>
                  <span className="w-full flex pb-2.5 items-center gap-1.5 px-4">
                    <span>
                      <MessagesSvg />
                    </span>
                    <p className="text-[#848282]">test123@gmail.com</p>
                  </span>
                </>
              ) : placeInfoMenu === "Video" ? (
                <></>
                ...
                <div className="px-4 flex flex-col gap-1.5 pt-1.5 relative overflow-hidden [&>div>div]:pb-10">
                  <span className="flex justify-between">
                    <h5 className="font-semibold text-sm">Popular places</h5>
        ...
        <div className="fixed w-full bottom-0 flex justify-between gap-2.5 pt-2.5  pb-5 px-4 bg-white z-10">
          <button className="rounded-[20px] border-2 border-main-blue p-4 text-main-blue w-full font-medium flex items-center justify-center gap-2.5">
            <GlobeSvg />
            <span>Website</span>
          </button>
            ...
            {profile ? (
              ...
              <>
                <p className="text-[#848282] text-center">
                  if you have not registered, a comment will be displayed after
                  verification
                </p>
